...
{
    public function FirstWebsite($searchQuery,$page)
    {
        ...
        if(searchArray($userSearch, $shortSearchCode) < 1) {
        // Define the $notMatchedRandomValue array
        $notMatchedRandomValue = [38, 16, 13, 15, 23, 43, 26, 40];

        // Get a random index from the $notMatchedRandomValue array
         $randIndex = array_rand($notMatchedRandomValue);
           
        // Get the random value from the $notMatchedRandomValue array
        $randValue = $notMatchedRandomValue[$randIndex];
           
         // Return the random value
         $exactSearchData = $randValue;
        }
        else {
    ...
    public function DecryptAudioListData(Request $request)
    {
        if (session()->has("firstWebsiteEncryptedData")) {
            ...
            $response = [
                "status" => "200",
                "info" =>
                    "If you want to buy this API just contact on github @JoshimOfficial", //Session delete success.
            ];
